...
export const zhToEnDict: Record<string, string> = {
  ...
  'ℹ️ 浏览器无法直接读取 TLS 证书与协议细节。本工具：① 提供一键跳转到 3 个权威在线检测平台；② 通过 Certificate Transparency Log（主源 certspotter，兜底 crt.sh）查询该域名的证书签发记录。':
    'ℹ️ Browsers cannot read TLS certificate/protocol details directly. This tool: (1) opens 3 authoritative scanners in a new window; (2) queries Certificate Transparency Log (primary: certspotter, fallback: crt.sh) for the domain\'s certificate issuance records.',
  ...
  '已吊销': 'Revoked',
  '来源：': 'Source: ',
  'example.com（或粘贴完整 URL）': 'example.com (or paste a full URL)',
  '查询证书': 'Query Certificates',
  '请输入有效的域名': 'Please enter a valid domain',
  '域名格式不正确（示例：example.com）': 'Invalid domain format (example: example.com)',
  'crt.sh 返回了非 JSON 内容（可能正在维护）': 'crt.sh returned non-JSON content (likely under maintenance)',
  'crt.sh 查询通常需要 10-60 秒，请耐心等待…': 'CT log queries usually take 10-60s, please wait…',
  '仅未过期': 'Only valid',
  '重试': 'Retry',
  '在 crt.sh 网页端打开': 'Open in crt.sh web UI',
  '在线检测（新窗口打开权威报告）': 'Online Scanners (opens in new window)',
  '业界标杆，A+~F 评级 / 协议 / 套件 / 漏洞全维度': 'Industry standard: A+~F grade, protocols, ciphers, vulnerabilities',
  'Mozilla 出品，含 HTTP 响应头安全评分': 'By Mozilla: includes HTTP security headers scoring',
  '国内站点，含中文报告与证书链可视化': 'China-based scanner with Chinese reports and certificate chain visualization',
  '域名：': 'Domain: ',
  '共': 'Total',
  '张证书记录': 'certificate records',
  '张有效': 'valid',
  '张已过期': 'expired',
  '重查': 'Re-query',
  '未在 CT 日志中查询到': 'No CT log records found for',
  '的证书记录': '',
  '有效证书（': 'Valid Certificates (',
  '个 SAN 域名': 'SAN names',
  '仅显示前 20 张，共': 'Showing top 20 of',
  '张有效证书': 'valid certificates',
  '历史已过期证书（': 'Expired Historical Certificates (',
  '仅显示前 30 条，共': 'Showing top 30 of',
  '条历史记录': 'historical records',
  '已过期': 'Expired',
  '仅剩': 'Only',
  '天': 'days',
  '天后过期': 'days left',
  '输入域名后可：① 一键查看 SSL Labs 等权威报告；② 查询 CT 日志中的所有证书签发记录':
    'Enter a domain to: (1) jump to SSL Labs and other authoritative reports; (2) query all certificate issuance records from CT logs',

  // === 网络测速工具 ===
  // Banner 太长（含 HTML），不进 zhEnDict，由代码直接显示
  '开始测试': 'Start Test',
  '重新测试': 'Restart',
  '点击"开始测试"运行': 'Click "Start Test" to run',
  '测延迟…': 'Measuring latency…',
  '测下载…': 'Measuring download…',
  '测上传…': 'Measuring upload…',
  '测试完成': 'Test completed',
  '测试失败': 'Test failed',
  '测速失败：': 'Speed test failed: ',
  '接入 Cloudflare': 'Connected to Cloudflare',
  '当前浏览器不支持 ReadableStream': 'This browser does not support ReadableStream',
  '上传失败（网络错误或被中止）': 'Upload failed (network error or aborted)',
  '延迟测试中': 'Latency Test',
  '延迟': 'Latency',
  '抖动': 'Jitter',
